...
#     c.save()
# # def create_pdf(text, filename):
# #     """Создание PDF через pandas + xlsxwriter (без reportlab)"""
# #     # Создаем DataFrame с текстом
# #     df = pd.DataFrame({'Сообщение': [text]})
# #
# #     # Создаем PDF через Excel стиль
# #     writer = pd.ExcelWriter(filename, engine='xlsxwriter')
# #     df.to_excel(writer, sheet_name='Сообщение', index=False)
# #
# #     # Настраиваем ширину колонок
# #     worksheet = writer.sheets['Сообщение']
...
# #
# #
# # def create_excel_file(text, filename):
# #     """Создание Excel файла вместо PDF"""
# #     df = pd.DataFrame({
...
# #
# #
# # def send_yandex_email(sender_full, app_password, recipient, text, file_path):
# #     """Отправка письма через Яндекс"""
# #     login = sender_full.split('@')[0]
# #
# #     msg = MIMEMultipart()
...
# #     # Текст письма
# #     body = st.session_state.get('email_body',
# #                                 "Здравствуйте!\n\nВо вложении Excel файл с вашим сообщением.\n\nС уважением,\nОтправитель")
# #     msg.attach(MIMEText(body, 'plain', 'utf-8'))
# #
# #     # Прикрепляем файл
# #     with open(file_path, 'rb') as f:
# #         attach = MIMEApplication(f.read(), _subtype='vnd.ms-excel')
# #         attach.add_header('Content-Disposition', 'attachment',
# #                           filename=f"message_{recipient.split('@')[0]}.xlsx")
# #         msg.attach(attach)
# #
# #     # Отправка
# #     server = smtplib.SMTP('smtp.yandex.ru', 587)
...
# # with st.sidebar:
# #     st.header("⚙️ Настройки отправителя")
# #     sender_email = st.text_input("📧 Ваш Яндекс email", placeholder="your.email@yandex.ru")
# #     app_password = st.text_input("🔑 Пароль приложения", type="password", placeholder="Пароль из Яндекса")
# #
# #     st.markdown("---")
# #     st.header("📝 Настройки письма")
# #     email_subject = st.text_input("✉️ Тема письма", value="Важное сообщение", key="email_subject")
# #     email_body = st.text_area("📄 Текст письма",
# #                               value="Здравствуйте!\n\nВо вложении Excel файл с вашим сообщением.\n\nС уважением,\nОтправитель",
...
# #     col_send1, col_send2, col_send3 = st.columns([1, 2, 1])
# #     with col_send2:
# #         if st.button("🚀 НАЧАТЬ РАССЫЛКУ", type="primary", use_container_width=True,
# #                      disabled=not (sender_email and app_password)):
# #
# #             df = st.session_state['df']
...
# #                     progress = (index + 1) / len(df)
# #                     progress_bar.progress(progress, text=f"Отправка {index + 1}/{len(df)}: {recipient}")
# #                     status_text.text(f"📨 Отправляем письмо {recipient}...")
# #
# #                     try:
# #                         # Создаем Excel файл
# #                         excel_name = f"temp_{index}.xlsx"
# #                         excel_path = os.path.join(temp_folder, excel_name)
# #                         create_excel_file(text, excel_path)
# #
# #                         # Отправляем
# #                         send_yandex_email(sender_email, app_password, recipient, text, excel_path)
# #                         success_count += 1
# #                         results.append({"email": recipient, "status": "✅ Успешно"})
# #                     except Exception as e:
# #                         fail_count += 1
# #                         results.append({"email": recipient, "status": "❌ Ошибка", "error": str(e)[:50]})
# #
...
# #                 col_res1, col_res2, col_res3 = st.columns(3)
# #                 col_res1.metric("✅ Успешно", success_count)
# #                 col_res2.metric("❌ Ошибок", fail_count)
# #                 col_res3.metric("📧 Всего", len(df))
# #
...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(0, 10, 'Сообщение', 0, 1, 'C')
        self.ln(10)
...
def create_pdf(text, filename):
    """Создание PDF с помощью FPDF2 (без зависимостей)"""
    pdf = PDF()
    pdf.add_page()
...
# --- ИНТЕРФЕЙС (полностью идентичен вашему) ---
st.title("📧 Массовая рассылка писем через Яндекс")
st.markdown("---")
